...
# @login_required # We will replace this with Firebase auth
def admin_test():
...
def requires_firebase_auth(f):
    def decorated(*args, **kwargs):
        ...
        try:
            ...
            g.firebase_user_id = decoded_token['uid']
...
def register_firebase_user():
    if request.method == 'POST':
        ...
        # username = data.get('username') # Uncomment if you send username
        ...
        if user:
            ...
            # db.session.commit() # Assuming your model functions handle commits
            return jsonify({'message': 'User linked with Firebase UID'}), 200
        else:
            # Registration not allowed for this email
            return jsonify({'message': 'Registration not allowed for this email'}), 403
